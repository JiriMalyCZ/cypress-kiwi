describe('Primary passenger - high priority tests and positive scenarios', () => {
  ...
  it('Fill invalid first name and surname -> values are not filled', () => {
    cy.get('input[name="firstname"]').type('123?').should('have.value','')
    cy.get('input[name="lastname"]').type('456&').should('have.value','')
  })
...
//beforeEach(() => {
 ...
 //const body = (response.body)
  ...
  //cy.visit('https://www.kiwi.com/en/booking?token={{token}}')
  //cy.get('button[class="ButtonPrimitive__StyledButtonPrimitive-sc-1lbd19y-0 gUUZvS"]').click()